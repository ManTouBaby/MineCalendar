...
public class LunarCalendar {
    ...
    private String solarFestival = "";//公历节日

    private String lunarFestival = "";//农历节日
...
//        }
    ...
    final static String[] lunarString1 = {
            "一", "二", "三", "四", "五", "六", "七", "八", "九", "十"
    };
    final static String[] lunarString2 = {
            "初", "十", "廿", "卅", "正", "腊", "冬", "闰"
    };
    ...
    public String getLunarDay() {
        String lunarDay = null;

        if (lday == 1) {
            lunarDay = chineseMonthNumber[lmonth - 1] + "月";
        } else if (lday <= 10) {
            lunarDay = lunarString2[0] + lunarString1[lday - 1];
        } else if (lday <= 19) {
            lunarDay = lunarString2[1] + lunarString1[lday % 10 - 1];
        } else if (lday == 20) {
            lunarDay = lunarString2[2];
        } else if (lday <= 29) {
            lunarDay = lunarString2[2] + lunarString1[lday % 10 - 1];
        } else if (lday == 30) {
            lunarDay = lunarString2[3];
        }
        if (solarTerms != null) lunarDay = solarTerms;

        if (solarFestival != null) lunarDay = solarFestival;

        if (lunarFestival != null) lunarDay = lunarFestival;
